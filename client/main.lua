...
RegisterNetEvent('srm-inventory:client:requiredItems', function(items, bool)
    local itemTable = {}
    if bool then
...
RegisterNetEvent('srm-inventory:client:hotbar', function(items)
...
RegisterNetEvent('srm-inventory:client:closeInv', function()
    SendNUIMessage({
        action = 'close',
    })
end)

RegisterNetEvent('srm-inventory:client:updateInventory', function()
    SendNUIMessage({
        action = 'update',
...
RegisterNetEvent('srm-inventory:client:ItemBox', function(itemData, type, amount)
    SendNUIMessage({
        action = 'itemBox',
        item = itemData,
        type = type,
...
RegisterNetEvent('srm-inventory:server:RobPlayer', function(TargetId)
    SendNUIMessage({
        action = 'RobMoney',
        TargetId = TargetId,
    })
...
RegisterNetEvent('srm-inventory:client:openInventory', function(items, other)
...
RegisterNetEvent('srm-inventory:client:giveAnim', function()
...
RegisterNUICallback('AttemptPurchase', function(data, cb)
    QBCore.Functions.TriggerCallback('srm-inventory:server:attemptPurchase', function(canPurchase)
        cb(canPurchase)
    end, data)
end)
...
RegisterNUICallback('CloseInventory', function(data, cb)
    ...
    if data.name then
        ...
        TriggerServerEvent('srm-inventory:server:closeInventory', data.name)
    elseif CurrentDrop then
        TriggerServerEvent('srm-inventory:server:closeInventory', CurrentDrop)
        CurrentDrop = nil
    end
    cb('ok')
end)
...
RegisterNUICallback('UseItem', function(data, cb)
    TriggerServerEvent('srm-inventory:server:useItem', data.item)
    cb('ok')
end)

RegisterNUICallback('SetInventoryData', function(data, cb)
    TriggerServerEvent('srm-inventory:server:SetInventoryData', data.fromInventory, data.toInventory, data.fromSlot, data.toSlot, data.fromAmount, data.toAmount)
    cb('ok')
end)
...
RegisterNUICallback('GiveItem', function(data, cb)
    ...
    if player ~= -1 and distance < 3 then
        ...
        QBCore.Functions.TriggerCallback('srm-inventory:server:giveItem', function(success)
            cb(success)
...
CreateThread(function()
    exports['qb-target']:AddTargetModel(Config.VendingObjects, {
        options = {
            {
                type = 'server',
                event = 'srm-inventory:server:openVending',
...
for i = 1, 5 do
    RegisterCommand('slot_' .. i, function()
        ...
        TriggerServerEvent('srm-inventory:server:useItem', itemData)
    end, false)
