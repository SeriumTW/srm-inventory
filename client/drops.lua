...
function GetDrops()
    QBCore.Functions.TriggerCallback('srm-inventory:server:GetCurrentDrops', function(drops)
        if drops then
                        ...
                        options = {
                            {
                                ...
                                action = function()
                                    TriggerServerEvent('srm-inventory:server:openDrop', k)
                                    CurrentDrop = dropId
                                end,
                            },
                        },
...
RegisterNetEvent('srm-inventory:client:removeDropTarget', function(dropId)
...
RegisterNetEvent('srm-inventory:client:setupDropTarget', function(dropId)
    while not NetworkDoesNetworkIdExist(dropId) do Wait(10) end
    local bag = NetworkGetEntityFromNetworkId(dropId)
    while not DoesEntityExist(bag) do Wait(10) end
        ...
        options = {
            {
                ...
                label = Lang:t('menu.o_bag'),
                action = function()
                    TriggerServerEvent('srm-inventory:server:openDrop', newDropId)
...
RegisterNUICallback('DropItem', function(item, cb)
    QBCore.Functions.TriggerCallback('srm-inventory:server:createDrop', function(dropId)
        if dropId then
            while not NetworkDoesNetworkIdExist(dropId) do Wait(10) end
            local bag = NetworkGetEntityFromNetworkId(dropId)
    ...
    while true do
        if holdingDrop then
            if IsControlJustPressed(0, 47) then
                ...
                TriggerServerEvent('srm-inventory:server:updateDrop', heldDrop, coords)
                holdingDrop = false
                bagObject = nil
                heldDrop = nil
            end
