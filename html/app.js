const InventoryContainer = Vue.createApp({
  ...
  methods: {
    ...
    async closeInventory() {
      ...
      try {
        await axios.post('https://srm-inventory/CloseInventory', {
          name: inventoryName,
        });
      } catch (error) {
    ...
    endDrag(event) {
      ...
      const targetInventoryContainer = event.target.closest(
        '.inventory-container'
      );
      if (
    ...
    async handleDropOnInventoryContainer() {
      if (
        ...
        try {
          const response = await axios.post('https://srm-inventory/DropItem', {
            ...newItem,
    ...
    async handlePurchase(targetSlot, sourceSlot, sourceItem, transferAmount) {
      try {
        const response = await axios.post(
          'https://srm-inventory/AttemptPurchase',
          {
            item: sourceItem,
    ...
    async useItem(item) {
      ...
      if (playerItemKey) {
        try {
          await axios.post('https://srm-inventory/UseItem', {
            inventory: 'player',
            item: item,
          });
          if (item.shouldClose) {
    ...
    async giveItem(item, quantity) {
      if (item && item.name) {
        ...
        if (playerHasItem) {
          ...
          try {
            await axios.post('https://srm-inventory/GiveItem', {
              item: selectedItem,
              amount: amountToGive,
          ...
          console.error(
            'Player does not have the item in their inventory. Item cannot be given.'
          );
        }
      }
      this.showContextMenu = false;
    ...
    inventoryError(slot) {
      ...
      axios.post('https://srm-inventory/PlayDropFail', {}).catch((error) => {
    ...
    openWeaponAttachments() {
      ...
      if (!this.showWeaponAttachments) {
        ...
        axios
          .post(
            'https://srm-inventory/GetWeaponData',
            JSON.stringify({
    ...
    removeAttachment(attachment) {
      ...
      axios
        .post(
          'https://srm-inventory/RemoveAttachment',
          JSON.stringify({
    ...
    postInventoryData(
      ...
      axios
        .post('https://srm-inventory/SetInventoryData', {
