    ...
    if player then
        if itemData then
            ...
            if AddItem(id, itemData['name'], amount, false, info, 'give item command') then
                ...
                TriggerClientEvent('srm-inventory:client:ItemBox', id, itemData, 'add', amount)
                if Player(id).state.inv_busy then TriggerClientEvent('srm-inventory:client:updateInventory', id) end
            else
    ...
    for _ = 1, 10, 1 do
        ...
        if emptySlot then
            if AddItem(source, randitem.name, amount, emptySlot, false, 'random items command') then
                TriggerClientEvent('srm-inventory:client:ItemBox', source, QBCore.Shared.Items[randitem.name], 'add')
...
RegisterCommand('hotbar', function(source)
    ...
    TriggerClientEvent('srm-inventory:client:hotbar', source, hotbarItems)
end, false)
...
RegisterCommand('inventory', function(source)
    ...
    if not QBPlayer then return end
    if not QBPlayer or QBPlayer.PlayerData.metadata['isdead'] or QBPlayer.PlayerData.metadata['inlaststand'] or QBPlayer.PlayerData.metadata['ishandcuffed'] then return end
    QBCore.Functions.TriggerClientCallback('srm-inventory:client:vehicleCheck', source, function(inventory, class)
