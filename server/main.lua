...
RegisterNetEvent('srm-inventory:server:openVending', function(data)
    local src = source
...
RegisterNetEvent('srm-inventory:server:closeInventory', function(inventory)
    local src = source
    ...
    if Drops[inventory] then
        ...
        if #Drops[inventory].items == 0 and not Drops[inventory].isOpen then -- if no listeed items in the drop on close
            TriggerClientEvent('srm-inventory:client:removeDropTarget', -1, Drops[inventory].entityId)
            Wait(500)
...
RegisterNetEvent('srm-inventory:server:useItem', function(item)
    local src = source
    ...
    if itemData.type == 'weapon' then
        ...
        TriggerClientEvent('srm-inventory:client:ItemBox', src, itemInfo, 'use')
    elseif itemData.name == 'id_card' then
        UseItem(itemData.name, src, itemData)
        TriggerClientEvent('srm-inventory:client:ItemBox', source, itemInfo, 'use')
...
RegisterNetEvent('srm-inventory:server:openDrop', function(dropId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    local playerPed = GetPlayerPed(src)
    ...
    drop.isOpen = true
    TriggerClientEvent('srm-inventory:client:openInventory', source, Player.PlayerData.items, formattedInventory)
end)

RegisterNetEvent('srm-inventory:server:updateDrop', function(dropId, coords)
    Drops[dropId].coords = coords
end)

RegisterNetEvent('srm-inventory:server:snowball', function(action)
    if action == 'add' then
        AddItem(source, 'weapon_snowball', 1, false, false, 'srm-inventory:server:snowball')
    elseif action == 'remove' then
        RemoveItem(source, 'weapon_snowball', 1, false, 'srm-inventory:server:snowball')
    end
end)

-- Callbacks

QBCore.Functions.CreateCallback('srm-inventory:server:GetCurrentDrops', function(_, cb)
    cb(Drops)
end)

QBCore.Functions.CreateCallback('srm-inventory:server:createDrop', function(source, cb, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then
        cb(false)
    ...
    if RemoveItem(src, item.name, item.amount, item.fromSlot, 'dropped item') then
        ...
        if not Drops[newDropId] then
            ...
            TriggerClientEvent('srm-inventory:client:setupDropTarget', -1, dropId)
        else
...
QBCore.Functions.CreateCallback('srm-inventory:server:attemptPurchase', function(source, cb, data)
...
QBCore.Functions.CreateCallback('srm-inventory:server:giveItem', function(source, cb, target, item, amount, slot, info)
    local player = QBCore.Functions.GetPlayer(source)
    ...
    TriggerClientEvent('srm-inventory:client:giveAnim', source)
    TriggerClientEvent('srm-inventory:client:ItemBox', source, itemInfo, 'remove', giveAmount)
    TriggerClientEvent('srm-inventory:client:giveAnim', target)
    TriggerClientEvent('srm-inventory:client:ItemBox', target, itemInfo, 'add', giveAmount)
    if Player(target).state.inv_busy then TriggerClientEvent('srm-inventory:client:updateInventory', target) end
    cb(true)
end)
...
RegisterNetEvent('srm-inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
